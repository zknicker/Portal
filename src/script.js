...
const poleLightMaterial = new THREE.MeshBasicMaterial({ color: 0xff892f });

// Portal Material
debugObject.portalColorStart = "#112636";
debugObject.portalColorEnd = "#d6eeff";
gui.addColor(debugObject, "portalColorStart").onChange(() => portalShaderMaterial.uniforms.uColorStart.value.set(debugObject.portalColorStart));
gui.addColor(debugObject, "portalColorEnd").onChange(() => portalShaderMaterial.uniforms.uColorEnd.value.set(debugObject.portalColorEnd));
const portalLightMaterial = new THREE.MeshBasicMaterial({ color: 0xd6eeff, side: THREE.DoubleSide });
...
gltfLoader.load("portal.glb", (gltf) => {
    ...
    gltf.scene.traverse((child) => {
        if (child.name == "poleLightA" || child.name == "poleLightB") {
            ...
            child.material = portalShaderMaterial;
        } else if (child.name == "baked") {
...
gui.add(firefliesShaderMaterial.uniforms.uSize, "value").min(0).max(500).step(1).name("firefliesSize");
